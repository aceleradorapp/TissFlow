...
module.exports = {
  async up(queryInterface) {
    // 1. Insere a ferramenta xml-template-builder (idempotente)
    const existing = await queryInterface.sequelize.query(
      "SELECT id FROM tools WHERE slug = 'xml-template-builder' LIMIT 1",
    ...
    if (existing.length > 0) {
      ...
      await queryInterface.sequelize.query(
        `INSERT INTO tools (name, slug, description, is_active, created_at, updated_at)
         VALUES (
           'Construtor de Modelos XML',
           'xml-template-builder',
           'Construa modelos XML personalizados a partir do XSD da ANS, ligando e desligando tags opcionais, e salve-os em sua biblioteca particular.',
           1, NOW(), NOW()
         )`,
      );
